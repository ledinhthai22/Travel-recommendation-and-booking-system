...
export default function ManagerCard({
    ...
    const isOnline = type === "tour" ? item.trangThai === 1 : item.trangThai;
    ...
    if (type === 'location') {
        ...
        statusText = isOnline ? 'Đang khai thác' : 'Ngưng khai thác';
    }
    ...
    if (type === 'hotel') {
        ...
        const mainImage = item.hinhAnh?.find(img => img.anhChinh);
        imageUrl = mainImage?.duongDanAnh || item.hinhAnh?.[0]?.duongDanAnh || item.anhDaiDien || '';

        subInfo1 = `${item.soSao} sao`;
        subInfo2 = item.soDienThoai;

        statusText = isOnline ? 'Đang Hợp tác' : 'Ngưng hợp tác';
    }


    if (type === "tour") {
        const mainImage = item.images?.find(img => img.anhChinh);
        imageUrl = mainImage?.duongDanAnh || item.images?.[0]?.duongDanAnh || "";

        displayName = item.tenTour;
        subInfo1 = item.tenLoaiTour;
        subInfo2 = `${item.ngay} ngày`;

        if (item.dem > 0) {
            subInfo2 += ` ${item.dem} đêm`;
        }

        statusText = item.trangThai === 1
            ? "Mở bán"
            : item.trangThai === 2
                ? "Tạm ngưng"
                : "Ngừng kinh doanh";
        ...
        <div className="group bg-white rounded-xl mt-[5px] mb-[0px] overflow-hidden border border-slate-200 hover:border-blue-400 transition-all duration-300 flex flex-col shadow-sm hover:shadow-md">
            ...
            <div className={`relative overflow-hidden ${!isOnline ? 'grayscale' : ''}`} style={{ height: '110px' }}>
                ...
                <div className="absolute top-0 left-2">
                    <span className={`px-2 py-1 rounded-md text-[8px] font-bold uppercase shadow-sm ${statusClass}`}>
                        {statusText}
                    </span>
                </div>
            ...
            <div className="p-3 flex flex-col flex-1">
                ...
                {type === 'hotel' && (
                    <>
                        <p className="flex items-center gap-1 text-[11px] font-bold text-slate-500 mt-1">
                ...
                <div className="mt-auto pt-3 border-t border-slate-100 flex justify-end">
                    <button
                        ref={buttonRef}
                        onClick={toggleMenu}
                        className="flex items-center gap-1 bg-[#0EA5E5] hover:bg-sky-600 text-white px-2.5 py-1.5 rounded-lg transition-all duration-200 active:scale-95 shadow-sm"
                    >
                        ...
                        <span className={`material-symbols-outlined transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} style={{ fontSize: '16px' }}>
                            arrow_drop_down
                        </span>
                    </button>
                </div>
