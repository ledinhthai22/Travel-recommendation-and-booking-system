    ...
    return (
        <div className="relative w-full" ref={ref}>
            <button
                type="button"
                onClick={() => setOpen((o) => !o)}
                className="flex w-full items-center justify-between gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-left text-sm text-slate-700 transition hover:border-[#0EA5E5] h-[42px]"
            >
                <span className="truncate tabular-nums text-xs">
                    {isApplied
...
export default function TourFilterBar({
    ...
    const hasActiveFilter = 
        province !== '' || 
        category !== '' || 
        (dayFilters && dayFilters.length > 0 && dayFilters[0] !== '') ||
        minPrice !== MIN ||
        maxPrice !== MAX;
        ...
        <div className="mb-8">
            <div className="flex flex-wrap items-end gap-4">
                ...
                <FilterField icon={Calendar} label="Số ngày đi" className="w-full sm:w-44">
                    <SelectField
                        value={dayFilters && dayFilters.length > 0 ? dayFilters[0] : ''}
                ...
                <FilterField icon={DollarSign} label="Khoảng giá" className="w-full sm:w-56">
                    <PriceRangeFilter
                        minPrice={minPrice}
                        maxPrice={maxPrice}
                ...
                {hasActiveFilter && onReset && (
                    <button
                        type="button"
                        onClick={handleReset}
                        className="flex items-center gap-1.5 rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-semibold text-slate-500 transition hover:bg-slate-100 hover:text-[#0EA5E5] shrink-0 h-[42px]"
                    >
                        <RotateCcw size={13} />
                        Xóa lọc
                    </button>
