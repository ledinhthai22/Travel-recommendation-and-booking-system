...
export default function LocationFormPage({ mode = 'add', initialData = null, onCancel, onSave }) {
    ...
    const validate = () => {
        ...
        if (!formData.loaiDiaDiem) {
            newErrors.loaiDiaDiem = 'Vui lòng tích chọn một loại trong bảng quản lý.';
            setActiveTab('category'); // Nhảy sang tab 2 nếu quên chọn
        }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-6 pt-4 border-t border-slate-100">
                            ...
                            {isViewMode ? (
                                ...
                                <Dropdown 
                                    ...
                                    options={typeData.map(item => ({ value: item.maLoaiDD, label: item.tenLoaiDD }))} 
                                    fullWidth
