    ...
    public class TourService : ITourService
    {
                    ...
                    if (schedule.ChiTietLichTrinh != null)
                    {
                        foreach (var detail in schedule.ChiTietLichTrinh)
                        {
                            ...
                            _context.CTLichTrinhs.Add(new CTLichTrinh
                            {
                                ...
                                LoaiHoatDong = detail.LoaiHoatDong,
        ...
        public async Task<bool> UpdateFullTourAsync(int tourId, TourFullCreateDTO dto, List<IFormFile> images, List<IFormFile> scheduleImages)
        {
            ...
            try
            {
                ...
                _tourCache.InvalidateTourDetail(tourId, oldSlug);
                if (existingTour.Slug != oldSlug)
                    _tourCache.InvalidateTourDetail(tourId, existingTour.Slug);
                _tourCache.InvalidateLists();
        ...
        {
            var cacheKey = _tourCache.DetailKey(tourId);
                        ...
                        .Select(l => new ScheduleReponseDTO
                        {
                            ...
                            ChiTietLichTrinhs = l.CTLichTrinhs
                                ...
                                .Select(ct => new ScheduleDetailsDTO
                                {
                                    ...
                                    LoaiHoatDong = ct.LoaiHoatDong
                                }).ToList()
                        }).ToList(),

                    ChuyenKhoiHanhs = t.ChuyenKhoiHanhs
        ...
        public async Task<TourReponseDTO?> GetTourDetailBySlugAsync(string slug)
        {
            ...
            var cacheKey = _tourCache.DetailSlugKey(normalizedSlug);
            if (_cache.TryGetValue(cacheKey, out TourReponseDTO? cached))
                return cached;
        ...
        public async Task<TourByLocationResponseDTO?> GetToursByLocationSlugAsync(string locationSlug)
        {
            ...
            var cacheKey = _tourCache.ListKey($"tour:bylocation:{normalizedSlug}");
        ...
        public async Task<bool> SoftDeleteTourAsync(int tourId)
        {
            ...
            _tourCache.InvalidateTourDetail(tourId, tour.Slug);
            _tourCache.InvalidateLists();
        ...
        public async Task<PageDTO<TourReponseDTO>> GetPagedTourAsync(int page, int pageSize, string? searchTerm, int? status)
        {
            ...
            var cacheKey = _tourCache.ListKey($"tour:paged:{page}:{pageSize}:{normalizedSearch}:{status}");
            if (_cache.TryGetValue(cacheKey, out PageDTO<TourReponseDTO>? cached))
                return cached!;
            ...
            var result = new PageDTO<TourReponseDTO>
            {
                Items = items,
        ...
        public async Task<List<TourSelectDTO>> GetToursForSelectAsync(string? keyword = null, int? status = null)
        {
            ...
            {
                var lowerKey = keyword.Trim().ToLower();
                query = query.Where(t => EF.Functions.Like(t.TenTour, $"%{lowerKey}%"));
            }
        ...
        public async Task<bool> SetMainImageAsync(int imageId)
        {
            ...
            // Chỉ update các ảnh đang là AnhChinh (thường chỉ có 1) thay vì load + lặp toàn bộ ảnh của tour
            await _context.HinhAnhTours
                .Where(x => x.MaTour == maTour && x.NgayXoa == null && x.AnhChinh && x.MaAnhTour != imageId)
                .ExecuteUpdateAsync(s => s.SetProperty(x => x.AnhChinh, false));
            ...
            var slug = await GetTourSlugAsync(maTour);
            _tourCache.InvalidateTourDetail(maTour, slug);
            _tourCache.InvalidateLists(); // Ảnh chính hiển thị trong các thẻ tour ở danh sách
        ...
        public async Task<bool> ChangeStatusAsync(int maTour, int trangThai)
        {
            ...
            // Trạng thái ảnh hưởng trực tiếp tới các danh sách (chỉ hiển thị tour TrangThai == 1)
            _tourCache.InvalidateTourDetail(maTour, tour.Slug);
            _tourCache.InvalidateLists();
        ...
        public async Task<PageDTO<FavoriteTourRepnoseDTO>> GetFavoriteToursAsync(int userId, int pageNumber = 1, int pageSize = 10)
        {
            ...
            var query = _context.DanhSachYeuThichs
                .AsNoTracking()
                .Where(y => y.MaNguoiDung == userId);
            ...
            var rawData = await query
                ...
                .Select(y => new
                {
                    ...
                    Dem = y.Tour.Dem,
                    Slug = y.Tour.Slug,
                    DiemDen = y.Tour.ChuyenKhoiHanhs.Select(c => c.DiemDen).FirstOrDefault(),
                    DuongDanAnh = y.Tour.HinhAnhTours.Where(a => a.AnhChinh == true).Select(a => a.DuongDanAnh).FirstOrDefault(),
                    GiaThapNhat = y.Tour.ChuyenKhoiHanhs
                        .SelectMany(c => c.GiaChuyens)
                        .Select(g => (decimal?)g.GiaNguoiLon)
                        .Min(),
                    ReviewCount = y.Tour.DanhGias.Count(),
                    DiemDanhGiaTrungBinh = y.Tour.DanhGias.Any() ? y.Tour.DanhGias.Average(d => (double)d.DiemDanhGia) : 0,
            ...
            var favoriteTours = rawData.Select(x => new FavoriteTourRepnoseDTO
            {
                ...
                GiaTour = x.GiaThapNhat ?? 0,
                ReviewCount = x.ReviewCount,
                DiemDanhGia = Math.Round(x.DiemDanhGiaTrungBinh, 1),
        ...
        public async Task<bool> DeleteFavoriteToursAsync(int userId, List<int> tourIds)
        {
            ...
            var affected = await _context.DanhSachYeuThichs
                .Where(y => y.MaNguoiDung == userId && tourIds.Contains(y.MaTour))
                .ExecuteDeleteAsync();
        ...
        public async Task<bool> AddFavoriteTourAsync(int userId, int tourId)
        {
            ...
            {
                // Race condition: đã được thêm bởi request khác gần như đồng thời
                return false;
            }
        ...
        {
            var cacheKey = _tourCache.ListKey($"tour:featured:{take}");
            ...
            var detailsById = details.ToDictionary(d => d.MaTour);

            var result = ids
             .Where(id => detailsById.ContainsKey(id))
        ...
        {
            var cacheKey = _tourCache.ListKey($"tour:newlyupdated:{take}");
            if (_cache.TryGetValue(cacheKey, out List<TourCardResponseDTO>? cached))
                return cached!;
        ...
        public async Task<List<TourCardResponseDTO>> GetToursByFeaturedDestinationAsync(string diemDen, int take = 6)
        {
            ...
            var cacheKey = _tourCache.ListKey($"tour:bydestination:{normalizedDiemDen}:{take}");
            if (_cache.TryGetValue(cacheKey, out List<TourCardResponseDTO>? cached))
                return cached!;
            ...
            var result = await _context.Tours
                ...
                .Where(t => t.TrangThai == 1
                         ...
                         && t.ChuyenKhoiHanhs.Any(c =>
                             c.NgayXoa == null
                             && EF.Functions.Like(c.DiemDen, $"%{normalizedDiemDen}%")
                             && c.NgayKhoiHanh >= now))
                .Select(t => new
                {
                    Tour = t,
        ...
        {
            var cacheKey = _tourCache.ListKey($"tour:mostbooked:{take}");
            if (_cache.TryGetValue(cacheKey, out List<TourCardResponseDTO>? cached))
                return cached!;
        ...
        {
            var cacheKey = _tourCache.ListKey($"tour:related:{maTour}:{take}");
            if (_cache.TryGetValue(cacheKey, out List<TourCardDTO>? cached))
                return cached!;

            var diemDenChinh = await _context.ChuyenKhoiHanhs
                .AsNoTracking()
                .Where(x => x.MaTour == maTour && x.NgayXoa == null)
                .Select(x => x.DiemDen)
                .FirstOrDefaultAsync();

            var maLoaiTourHienTai = await _context.Tours
                .AsNoTracking()
                .Where(t => t.MaTour == maTour)
                .Select(t => (int?)t.MaLoaiTour)
                .FirstOrDefaultAsync();

            if (maLoaiTourHienTai == null)
            ...
            var relatedTours = await _context.Tours
                ...
                .Where(t =>
                    ...
                    t.NgayXoa == null)
                .OrderByDescending(t =>
                    t.MaLoaiTour == maLoaiTourHienTai ? 100 : 0)
                .ThenByDescending(t =>
                    t.ChuyenKhoiHanhs.Any(c =>
        ...
        {
            var cacheKey = _tourCache.ListKey($"tour:relatedbyhotel:{maKhachSan}");
            if (_cache.TryGetValue(cacheKey, out List<TourCardResponseDTO>? cached))
                return cached!;
